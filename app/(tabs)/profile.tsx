import { ArrowLeft, ChevronRight, CircleUserRound, Info, LogOut, Download, CloudUpload, Wallet } from 'lucide-react-native';
...
export default function ProfileTab() {
  return (
    <View style={styles.page}>
      <View style={styles.top}><ArrowLeft color="#fff" size={28} /><Text style={styles.title}>Profile</Text><View style={{ width: 28 }} /></View>
      ...
      <View style={styles.list}>
        {items.map(({ key, title, icon: Icon }) => (
          <TouchableOpacity key={key} style={styles.item}><View style={styles.ic}><Icon size={22} color="#111" /></View><Text style={styles.itemText}>{title}</Text><ChevronRight color="#111" size={26} /></TouchableOpacity>
        ))}
      </View>
...
const styles = StyleSheet.create({
  ...
  name: { color: '#fff', textAlign: 'center', fontSize: 64/2, fontWeight: '800', marginTop: 20 },
  email: { color: '#9ca3af', textAlign: 'center', fontSize: 42/2, marginTop: 6 },
  ...
  itemText: { flex: 1, marginLeft: 14, color: '#0b0b0d', fontSize: 46/2, fontWeight: '700' },
  logout: { marginTop: 18, borderWidth: 2, borderColor: '#fff', borderRadius: 20, padding: 16, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 10 },
  logoutText: { color: '#fff', fontSize: 46/2, fontWeight: '800' },
